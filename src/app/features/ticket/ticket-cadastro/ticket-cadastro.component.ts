...
@Component({
  ...
  styleUrls: ['./ticket-cadastro.component.css']
})
export class TicketCadastroComponent implements OnInit {
  ...
  editarTicket(codigo: number) {
    return this.ticketService.editar(codigo)
    .then((ticket) => {
...
showComplementoTicket() {
  ...
  this.complementoTicket.tipoComplemento = 2;
  this.displayModalComplemento = true;
}
...
responderComplemento() {
  this.loaderService.show();
  this.complementoTicket.codigoUsuarioResposta = this.user.id;
  this.ticketService
    .responderComplemento(this.complementoTicket)
    .then((response) => {
      this.loaderService.hide();
      this.complementosTicket = response;
      this.messageService.add({
        severity: "success",
        detail: "Complemento respondido com sucesso!",
      });
    }).then( (res) => {
      this.editarTicket(this.ticket.id);
      this.displayModalComplemento = false;
