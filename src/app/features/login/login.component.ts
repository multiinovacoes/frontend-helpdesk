  ...
  onClickLogin() {
    this.userService.getUserByUserNameAndPassword(this.userName, this.password)
      .then(() => {
        ...
        this.user.codigoCliente = this.userService.jwtPayload.id_cliente;
        this.user.id = this.userService.jwtPayload.id_usuario;
        this.userContextService.setUser(this.user);
        if (this.user.codigoCliente == 0){
          this.routeStateService.add("Ticket", '/main/ticket-atendimento', null, true);
        }else{
